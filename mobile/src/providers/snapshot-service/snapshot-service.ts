  ...
  async getIndicators(areaCode: string,type:number,sector:ISector,showLoader,hideLoader) {
    ...
    try {
      ...
      let dataMap = new Map();
        ...
        let data: IDBData[]=[];
try{
  let datas=((await this.http.get('assets/data/'+areaCode+'.json').toPromise()) as IJsonData[])
  data=this.pouchdbService.convertJsonDataToIDBData(datas);
      }
      catch(e)
      {
        data=[];
  ...
  async getLineChartData(area: IArea,data:IDBData[]) {
    ...
    array.sort(this.utilService.sortMonthlyArray.bind(this))
    array.reverse();
...
try{
   ...
   let jsondata=  ((await this.http.get('assets/data/'+indicatorId+'.json').toPromise()) as IJsonData[])
  
    dbdata = this.pouchdbService.convertJsonDataToIDBData(jsondata);
    if(type==SnpashotProfile.RMNCHA)
    {
